...
| **Skill** | History からパターン抽出 → Skill ファイル生成 |
...
ユーザーが `pfl-forge operator`（または `pfl-forge` のみ）を実行したとき。
...
- `session_id` パラメータ: Runner から渡される。中断再開時に `--resume <session_id>` で前回のセッションを継続し、探索コンテキストを引き継ぐ
- モデル: complexity に応じて `models.implement`（low/medium）または `models.implement_complex`（high）
...
- モデル: `models.review`（default: sonnet）
...
- Execution Summary（Analyze 計画、コミットメッセージ、Review 結果の構造化サマリ）
- Observation（Review Agent が記録した横断的な気づき）

### 処理内容

- Execution Summary から Intent フロー全体を振り返る
...
History から繰り返しパターンを検出し、再利用可能な Skill ファイルを生成するエージェント群。3つのステップで構成される。

### 起動タイミング

Intent の `type: skill_extraction` で Runner が Flow `[observe, abstract, record]` を実行するとき。

### ステップ

#### observe

History エントリを分析し、繰り返されるパターンを検出する。

- 入力: `.forge/knowledge/history/*.yaml`
- モデル: `models.skill`（default: sonnet）
- 出力: `ObserveResult { patterns: Vec<ObservedPattern> }`
  - 各パターン: `name`, `description`, `frequency`, `examples`
- History が空の場合は Claude を呼ばずに空のパターンリストを返す

#### abstract

検出されたパターンを汎用的な Skill テンプレートに変換する。

- 入力: observe の出力パターン
- モデル: `models.skill`（default: sonnet）
- 出力: `AbstractResult { skills: Vec<SkillDraft> }`
  - 各 Skill: `name`, `description`, `instructions`
- パターンが空の場合は Claude を呼ばずに空の Skill リストを返す

#### record

Skill テンプレートをファイルに書き出す（Claude 呼び出しなし）。

- 出力: `.claude/skills/{name}/SKILL.md`
- ファイル形式: frontmatter (`description`) + instructions

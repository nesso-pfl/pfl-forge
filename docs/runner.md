...
resume 時に `sessions.implement` を `claude -p --resume <session_id>` として Implement Agent に渡す。Analyze の clarification 再開時は `sessions.analyze` を使用する。これにより前回の探索コンテキストを引き継ぎ、トークン消費を抑える。

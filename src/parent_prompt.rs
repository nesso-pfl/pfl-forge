...
pub fn build_system_prompt(config: &Config) -> String {
    let repos_info: Vec<String> = config
        ...
        .map(|r| {
            format!(
                "- {name} ({github}): path={path}, test=`{test}`, label={label}, base={base}",
                name = r.name,
                github = r.github,
                path = r.path.display(),
                test = r.test_command,
    ...
    format!(
        r#"You are the parent agent for pfl-forge, a multi-agent issue processor.
You manage issue processing by running pfl-forge CLI commands via Bash.

## Available commands

- `pfl-forge run` — Process pending issues (fetch, triage, execute, integrate)
  - `--repo <name>` — Process only a specific repo
  - `--resume` — Resume failed/interrupted issues
...
- `pfl-forge clean` — Clean up worktrees for completed issues
- `pfl-forge watch` — Daemon mode: poll and process periodically

## Clarification workflow

When a worker cannot resolve an issue due to ambiguity, it creates a clarification request.
...
After answering, the issue is reset to pending and will be re-processed on the next `pfl-forge run`.
...
- Report results (PRs created, errors) back to the user clearly."#,
...
pub fn build_initial_message(config: &Config, state: &StateTracker) -> Result<String> {
    ...
    if !pending.is_empty() {
        ...
        msg.push_str("\nNo pending clarifications. You can run `pfl-forge run` to process new issues or check status.");
    }

...
pub async fn integrate_one(
    ...
    // Run tests on base branch before rebase
    info!("running base branch tests for {issue}");
    let wt = worktree_path.clone();
    let bb = base_branch.clone();
    let branch_clone = branch.clone();
    let test_cmd = repo_config.test_command.clone();
    let base_tests_passed = tokio::task::spawn_blocking(move || -> crate::error::Result<bool> {
        git::branch::checkout_detached(&wt, &format!("origin/{bb}"))?;
        let passed = crate::pipeline::execute::run_tests(&wt, &test_cmd)?;
        git::branch::checkout_branch(&wt, &branch_clone)?;
        Ok(passed)
    })
    .await
    .map_err(|e| crate::error::ForgeError::Git(format!("spawn_blocking: {e}")))??;

    if base_tests_passed {
        info!("base branch tests pass for {issue}");
    } else {
        warn!("base branch tests already failing for {issue}");
    }
    ...
    if let Err(e) = rebase_result {
        ...
        // Push the pre-rebase branch as-is
        let wt = worktree_path.clone();
        let b = branch.clone();
    ...
    if !test_passed {
        let (message, status) = if base_tests_passed {
            // base=pass, feature=fail → true regression
            info!("regression detected for {issue}: tests pass on base but fail on feature");
            (
                "**Tests failed after rebase.** This is a regression introduced by the implementation. Manual review needed.",
                IssueStatus::TestFailure,
            )
        } else {
            // base=fail, feature=fail → pre-existing failure
            info!("pre-existing test failures for {issue}: tests fail on both base and feature");
            (
                "**Tests failed after rebase.** Pre-existing test failures on base branch — this may not be caused by the implementation. Manual review needed.",
                IssueStatus::TestFailure,
            )
        };
    ...
    let review_result = tokio::task::spawn_blocking(move || {
        review::review(&issue_clone, &deep_clone, &config_clone, &review_runner, &wt, &bb, base_tests_passed)
    })
    .await
    ...
    match review_result {
        Ok(result) if !result.approved => {
            ...
            let suggestions_text = if result.suggestions.is_empty() {
                ...
                format!(
                    "\n\n**Suggestions:**\n{}",
                    result.suggestions.iter().map(|s| format!("- {s}")).collect::<Vec<_>>().join("\n")
                )
            };
            let pr_body = format!(
